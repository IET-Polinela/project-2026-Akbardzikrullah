...
class WorkflowStateTests(APITestCase):
    ...
    def test_WF_02_tidak_bisa_edit_laporan_yang_sudah_reported(self):
        ...
        url = reverse('report-detail', args=[self.laporan_reported.pk])
        payload = {
            'title': 'Judul Tidak Boleh Berubah',
            'category': self.laporan_reported.category,
            'description': 'Deskripsi ini seharusnya ditolak.',
        ...
        self.assertEqual(
            ...
            "Laporan REPORTED tidak boleh diedit lagi oleh warga"
        )
    ...
    def test_WF_05_laporan_resolved_tidak_bisa_diubah(self):
        ...
        url = reverse('report-detail', args=[self.laporan_resolved.pk])
        payload = {
            'title': 'Resolved Diubah',
            'category': self.laporan_resolved.category,
            'description': 'Perubahan pada laporan selesai harus ditolak.',
...
class AdminWorkflowTests(TestCase):
    ...
    def test_WF_03_admin_mengubah_status_reported_ke_verified(self):
        ...
        url = reverse('main_app:update_status', args=[self.laporan_reported.pk])
        response = self.client.post(url, {'status': 'VERIFIED'})

        self.assertEqual(
            response.status_code,
            status.HTTP_302_FOUND,
            "Update status melalui portal admin seharusnya redirect setelah sukses"
        )
    ...
    def test_WF_04_tidak_ada_transisi_langsung_ke_resolved_dari_reported(self):
        ...
        response = self.client.get(reverse('main_app:report_list'))

        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertContains(response, 'name="status" value="VERIFIED"')
        self.assertContains(response, 'Verifikasi')
        self.assertNotContains(response, 'name="status" value="RESOLVED"')
